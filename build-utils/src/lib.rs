...
pub fn emit_build_info(prefix: &str) {
    println!("cargo:rerun-if-changed=build.rs");
    println!("cargo:rerun-if-changed=.git/HEAD");
    println!("cargo:rerun-if-changed=.git/refs/heads");

    set_env(
        &format!("{prefix}_RUSTC_VERSION"),
        rustc_version().unwrap_or_else(|| "unknown".into()),
    );
    set_env(
        &format!("{prefix}_GIT_COMMIT"),
        git_commit().unwrap_or_else(|| "unknown".into()),
    );
    set_env(&format!("{prefix}_BUILD_TIME"), build_timestamp());
    set_env(
        &format!("{prefix}_TARGET_OS"),
        env::var("CARGO_CFG_TARGET_OS").unwrap_or_else(|_| "unknown".into()),
    );
    set_env(
        &format!("{prefix}_TARGET_ARCH"),
...
fn git_commit() -> Option<String> {
    ...
    let dirty = git_has_staged_changes().unwrap_or(false);
    let suffix = if dirty { "-dirty" } else { "" };
    Some(format!("{}{}", hash, suffix))
}

fn git_has_staged_changes() -> Option<bool> {
    let output = Command::new("git")
